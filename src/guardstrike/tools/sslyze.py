        ...
        try:
                ...
                if cert_deployments:
                    ...
                    if verified_chain:
                        ...
                        results["certificate"] = {
                            ...
                            "signature_algorithm": leaf_cert.get("signature_algorithm_oid", {}).get(
                                "name", ""
                            ),
                        }

                    # Certificate validation
                    validation_result = cert_deployment.get(
            ...
            for vuln_key, vuln_name in vuln_checks.items():
                if vuln_key in scan_commands:
                    ...
                    if is_vulnerable:
                        results["vulnerabilities"].append(
                            {"name": vuln_name, "severity": "critical"}
                        )
            ...
            if "http_headers" in scan_commands:
                ...
                if strict_transport_security:
                    results["security_headers"]["HSTS"] = strict_transport_security.get(
                        "max_age", 0
                    )
                else:
