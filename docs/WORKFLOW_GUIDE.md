# GuardStrike Developer Guide: Creating Custom Workflows

This guide explains how to create custom penetration testing workflows for GuardStrike.
...
GuardStrike workflows are YAML files that define:
...
```bash
# Basic usage
python -m guardstrike workflow run --name my_workflow --target example.com

# With config file
python -m guardstrike workflow run \
  --name comprehensive_assessment \
  --target example.com \
  --config config/guardstrike.yaml
```
...
# Check GuardStrike recognizes it
python -m guardstrike workflow list
```
