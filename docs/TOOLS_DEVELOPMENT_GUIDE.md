# GuardStrike Developer Guide: Creating Custom Tools

This guide walks you through creating custom pentesting tools for GuardStrike.
...
GuardStrike tools follow a consistent architecture:
...
def _map_severity(self, raw_severity: str) -> str:
    """Map tool's severity to GuardStrike's standard levels"""
    severity_map = {
        "critical": "critical",
        "high": "high",
        "medium": "medium",
...
async def main():
    config = load_config("config/guardstrike.yaml")
    tool = MyToolTool(config)
