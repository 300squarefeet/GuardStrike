...
# GuardStrike CLI – Skill Reference

## 1. Project Overview

**GuardStrike** (v2.0) is a Python 3.11+ CLI application that automates penetration testing workflows using a multi-agent AI system. It is designed for **authorized** security assessments only.
...
├── config/           # guardstrike.yaml configuration file
...
Switch provider via `config/guardstrike.yaml` or `--provider` CLI flag.

---

## 3. CLI Commands

Run with `python -m guardstrike <command>` (or `guardstrike <command>` after installation).

| Command | Purpose |
|---|---|
| `init` | Create/validate `config/guardstrike.yaml` |
...
python -m guardstrike workflow run --name web_pentest --target https://target.example.com

# Full network assessment
python -m guardstrike workflow run --name network --target 192.168.1.0/24

# Autonomous AI-driven pentest
python -m guardstrike workflow run --name autonomous --target example.com
```
...
steps:
  - name: http_discovery
    ...
    parameters:             # Override config/guardstrike.yaml defaults
      tech_detect: true
      threads: 100
...
**Parameter Priority:** Workflow YAML > `config/guardstrike.yaml` > Tool defaults
...
GuardStrike works with a subset of tools available; the AI adapts based on what is installed.

---

## 6. Configuration (`config/guardstrike.yaml`)
...
4. Add the config block to `config/guardstrike.yaml` under `ai:`
...
> ⚠️ GuardStrike is designed **exclusively** for authorized security testing and educational purposes.  
